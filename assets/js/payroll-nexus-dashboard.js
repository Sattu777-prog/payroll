...
(function () {
    ...
    const DEFAULT_NOTIFICATIONS = [
        {
            id: 'notif-1',
            category: 'payroll',
            title: 'Bi-Weekly Payroll Cycle Batch Disbursed',
            desc: 'Batch #PN-9824 processed 12 staff disbursements totaling $12,304.11 with 100% direct-deposit verification.',
            time: '4m ago',
            unread: true,
            severity: 'success',
        ...
        },
        {
            id: 'notif-2',
            category: 'workforce',
            title: 'Workforce Roll Call Synchronized',
            desc: 'Morning biometric sign-in achieved 91.7% on-time presence (11/12 verified, 1 scheduled leave).',
            time: '26m ago',
            unread: true,
            severity: 'info',
        ...
        },
        {
            id: 'notif-3',
            category: 'workforce',
            title: 'Pending Leave Request Requires Review',
            desc: 'Marcus Vance submitted a 3-day Medical Leave request starting tomorrow. Supervisor signature required.',
            time: '1h ago',
            unread: true,
            severity: 'warning',
        ...
        },
        {
            id: 'notif-4',
            category: 'payroll',
            title: 'Multi-Currency Treasury Buffer Healthy',
            desc: 'Consolidated reserves at $842,600 (70% Crypto, 30% Fiat). Working capital runway at 18.4 months.',
            time: '3h ago',
            unread: false,
            severity: 'success',
            icon: 'fas fa-vault',
        ...
        },
        {
            id: 'notif-5',
            category: 'security',
            title: 'Cryptographic Session Token Upgraded',
            desc: 'Admin Nadia Rachel session key upgraded to TLS 1.3 zero-trust perimeter token with 2FA enforcement.',
            time: '5h ago',
            unread: false,
            severity: 'governance',
    ...
    let heliosNotifications = [];
    try {
        const savedNotifs = localStorage.getItem('helios_enterprise_notifs');
        if (savedNotifs) {
            heliosNotifications = JSON.parse(savedNotifs);
        } else {
            heliosNotifications = JSON.parse(JSON.stringify(DEFAULT_NOTIFICATIONS));
        }
    } catch (e) {
        heliosNotifications = JSON.parse(JSON.stringify(DEFAULT_NOTIFICATIONS));
    }
    ...
    function saveNotificationsState() {
        try {
            localStorage.setItem('helios_enterprise_notifs', JSON.stringify(heliosNotifications));
        } catch (e) {}
        syncNotificationBadges();
    }

    function syncNotificationBadges() {
        const unreadCount = heliosNotifications.filter(n => n.unread).length;
        const deskBadge = document.querySelector('#notifBtn .helios-icon-badge');
        const mobBadge = document.querySelector('#mobileNotifBtn .pn-icon-badge');
        const headerPill = document.getElementById('notifHeaderUnreadPill');

        if (deskBadge) {
            deskBadge.style.display = unreadCount > 0 ? 'block' : 'none';
        }
        if (mobBadge) {
            mobBadge.style.display = unreadCount > 0 ? 'block' : 'none';
        }
        if (headerPill) {
            headerPill.textContent = unreadCount > 0 ? `${unreadCount} Unread` : 'All Read';
            if (unreadCount === 0) {
                headerPill.style.background = 'rgba(98, 200, 138, 0.15)';
                headerPill.style.color = '#62c88a';
                headerPill.style.borderColor = 'rgba(98, 200, 138, 0.3)';
            } else {
    ...
    function updateFilterPillCounts() {
        const counts = {
            all: heliosNotifications.length,
            payroll: heliosNotifications.filter(n => n.category === 'payroll').length,
            workforce: heliosNotifications.filter(n => n.category === 'workforce').length,
            security: heliosNotifications.filter(n => n.category === 'security').length
        };
        ['all', 'payroll', 'workforce', 'security'].forEach(cat => {
            const countEl = document.querySelector(`.notif-filter-pill[data-filter="${cat}"] .notif-pill-count`);
            if (countEl) countEl.textContent = counts[cat] || 0;
        });
    }

    function renderNotificationsList() {
        const container = document.getElementById('heliosNotifList');
        if (!container) return;

        let filtered = heliosNotifications;
        if (currentNotifFilter !== 'all') {
            filtered = heliosNotifications.filter(n => n.category === currentNotifFilter);
        }
        ...
        if (filtered.length === 0) {
            container.innerHTML = `
                <div class="notif-empty-state">
                    ...
                    <div class="notif-empty-desc">There are no active alerts in this category. All payroll disbursements, attendance syncs, and compliance checks are steady.</div>
                    <button type="button" class="notif-header-btn" id="notifRestoreDemoBtn" style="margin-top:6px;">
                        <i class="fas fa-rotate-left"></i> Restore Demo Alerts
                    </button>
                </div>
            `;
            const restoreBtn = document.getElementById('notifRestoreDemoBtn');
            if (restoreBtn) {
                restoreBtn.addEventListener('click', () => {
                    heliosNotifications = JSON.parse(JSON.stringify(DEFAULT_NOTIFICATIONS));
                    saveNotificationsState();
                    renderNotificationsList();
                    showToastNotification('Sample notifications restored', 'info');
                });
            }
            return;
        }
        ...
        container.innerHTML = filtered.map(item => `
            <div class="helios-notif-card ${item.unread ? 'is-unread' : ''}" data-id="${item.id}">
                <div class="notif-card-icon-box ${item.severity}">
                    <i class="${item.icon}"></i>
                </div>
                <div class="notif-card-content">
                    <div class="notif-card-top">
                        <span class="notif-card-category ${item.category}">${item.category.toUpperCase()}</span>
                        <span class="notif-card-time"><i class="far fa-clock" style="font-size:10px;margin-right:3px;"></i>${item.time}</span>
                    </div>
                    <div class="notif-card-title">${item.title}</div>
                    <div class="notif-card-desc">${item.desc}</div>
                    <div class="notif-card-bottom">
                        <div>
                            ${item.actionLabel ? `
                                <button type="button" class="notif-action-btn" data-action="${item.actionTab}">
                                    ${item.actionLabel} <i class="fas fa-arrow-right" style="font-size:10px;"></i>
                                </button>
                            ` : ''}
                        </div>
                        <div class="notif-card-item-actions">
                            <button type="button" class="notif-mini-btn notif-toggle-read" title="${item.unread ? 'Mark as Read' : 'Mark as Unread'}">
                                <i class="fas ${item.unread ? 'fa-envelope-open' : 'fa-envelope'}"></i>
                            </button>
                            <button type="button" class="notif-mini-btn notif-delete-item" title="Dismiss Alert">
        ...
        container.querySelectorAll('.helios-notif-card').forEach(card => {
            const id = card.getAttribute('data-id');
            const item = heliosNotifications.find(n => n.id === id);

            const actionBtn = card.querySelector('.notif-action-btn');
            if (actionBtn && item) {
                actionBtn.addEventListener('click', (e) => {
                    ...
                    const modal = document.getElementById('heliosNotifModal');
                    if (modal) modal.classList.remove('open');

                    if (item.actionTab === 'admin') {
                        openAuthProfileModal('admin');
                    } else if (typeof window.switchToTab === 'function') {
                        window.switchToTab(item.actionTab);
                    }
                    showToastNotification(`Navigated to ${item.actionTab.toUpperCase()}`, 'info');
                });
            }

            const toggleBtn = card.querySelector('.notif-toggle-read');
            if (toggleBtn && item) {
            ...
            const deleteBtn = card.querySelector('.notif-delete-item');
            if (deleteBtn) {
                deleteBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    heliosNotifications = heliosNotifications.filter(n => n.id !== id);
                    saveNotificationsState();
                    renderNotificationsList();
                });
            }
    ...
    function openNotifModal() {
        ...
        if (!modal) {
            ...
            modal.className = 'modal-backdrop';
            modal.innerHTML = `
                <div class="modal-card helios-notif-modal-card">
                    <div class="helios-notif-header">
                        <div class="helios-notif-title-wrap">
                            <i class="fas fa-bell" style="color:#b68cff;font-size:17px;"></i>
                            <h3>Notifications & Alerts</h3>
                            <span class="notif-unread-badge" id="notifHeaderUnreadPill">3 Unread</span>
                        </div>
                        <div class="helios-notif-header-actions">
                            <button type="button" class="notif-header-btn" id="notifMarkAllReadBtn" title="Mark all alerts as read">
                                <i class="fas fa-check-double"></i> Mark all read
                            </button>
                            <button type="button" class="modal-close" id="closeHeliosNotifModal" aria-label="Close notifications"><i class="fas fa-xmark"></i></button>
                        </div>
                    </div>

                    <!-- Category Filters -->
                    <div class="helios-notif-filter-bar">
                        <button type="button" class="notif-filter-pill active" data-filter="all">
                            <span>All</span> <span class="notif-pill-count">5</span>
                        </button>
                        <button type="button" class="notif-filter-pill" data-filter="payroll">
                            <span>Payroll & Treasury</span> <span class="notif-pill-count">2</span>
                        </button>
                        <button type="button" class="notif-filter-pill" data-filter="workforce">
                            <span>Workforce</span> <span class="notif-pill-count">2</span>
                        </button>
                        <button type="button" class="notif-filter-pill" data-filter="security">
                            <span>Security</span> <span class="notif-pill-count">1</span>
                        </button>
                    </div>

                    <!-- Notifications List -->
                    <div class="helios-notif-body" id="heliosNotifList">
                        <!-- Rendered dynamically -->
                    </div>

                    <!-- Footer with live stream status -->
                    <div class="helios-notif-footer">
                        <div class="notif-live-telemetry">
                            <span class="notif-pulse-dot"></span>
                            <span>Live Ledger Telemetry Active</span>
                        </div>
                        <div style="display:flex;align-items:center;gap:8px;">
                            <button type="button" class="notif-header-btn" id="notifClearAllBtn" style="font-size:11px;padding:4px 8px;">
                                <i class="fas fa-trash-can" style="font-size:10px;"></i> Clear All
                            </button>
                        </div>
                    </div>
                </div>
            ...
            // Filter pills
            modal.querySelectorAll('.notif-filter-pill').forEach(pill => {
                pill.addEventListener('click', () => {
                    modal.querySelectorAll('.notif-filter-pill').forEach(p => p.classList.remove('active'));
                    pill.classList.add('active');
                    currentNotifFilter = pill.getAttribute('data-filter');
                    renderNotificationsList();
                });
            });
            ...
            document.getElementById('notifMarkAllReadBtn').addEventListener('click', () => {
                heliosNotifications.forEach(n => n.unread = false);
                saveNotificationsState();
                renderNotificationsList();
            ...
            document.getElementById('notifClearAllBtn').addEventListener('click', () => {
                ...
                saveNotificationsState();
                renderNotificationsList();
                showToastNotification('All notifications cleared', 'info');
            });
        }
    ...
    try {
        ...
        if (currentUserState.email === 'rachel_helios@gmail.com') {
            currentUserState.email = 'rachel_nexus@gmail.com';
    ...
    // ── ADMIN WORKFORCE DIRECTORY & GOVERNANCE STATE ──
    let adminWorkforceData = [
        { id: 'usr_1', name: 'Nadia Rachel', email: 'rachel_nexus@gmail.com', role: 'admin', dept: 'Executive Governance', status: 'Online', avatar: 'assets/images/avatar.jpg' },
        { id: 'usr_2', name: 'Alex Admin', email: 'admin@nexus.dev', role: 'admin', dept: 'System Infrastructure', status: 'Active', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80' },
        { id: 'usr_3', name: 'Maria Manager', email: 'manager@nexus.dev', role: 'manager', dept: 'People & Operations', status: 'Active', avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80' },
        { id: 'usr_4', name: 'Chen Wei', email: 'employee@nexus.dev', role: 'employee', dept: 'Distributed Systems', status: 'Active', avatar: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=150&auto=format&fit=crop&q=80' },
        { id: 'usr_5', name: 'Sattu Admin', email: 'sattubeb777@gmail.com', role: 'admin', dept: 'Cloud Architecture', status: 'Active', avatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80' }
    ];

    let isPayrollFrozen = false;
    let isStrict2fa = true;

    function renderAdminPanelContent() {
        const tbody = document.getElementById('adminUsersTableBody');
        if (!tbody) return;

        tbody.innerHTML = adminWorkforceData.map(user => `
            <tr>
                <td>
                    <div style="display:flex;align-items:center;gap:10px;">
                        <img src="${user.avatar}" alt="${escapeHtml(user.name)}" style="width:32px;height:32px;border-radius:50%;object-fit:cover;border:1px solid rgba(255,255,255,0.15);">
                        <div>
                            <div style="font-weight:600;color:#f1f0f5;font-size:12.5px;">${escapeHtml(user.name)}</div>
                            <div style="font-size:10.5px;color:#8f8d99;">ID: ${user.id}</div>
                        </div>
                    </div>
                </td>
                <td style="font-size:12px;color:#b68cff;">${escapeHtml(user.email)}</td>
                <td style="font-size:12px;color:#c0bfcf;">${escapeHtml(user.dept)}</td>
                <td>
                    <select class="admin-role-select" data-user-id="${user.id}" data-user-email="${user.email}" style="background:#13141c;color:#fff;border:1px solid rgba(255,255,255,0.15);border-radius:6px;padding:4px 8px;font-size:11.5px;font-weight:600;outline:none;cursor:pointer;">
                ...
                <td>
                    <span style="display:inline-flex;align-items:center;gap:5px;font-size:11px;font-weight:600;padding:2px 8px;border-radius:12px;background:rgba(98,200,138,0.12);color:#62c88a;border:1px solid rgba(98,200,138,0.3);">
                        <span style="width:5px;height:5px;border-radius:50%;background:#62c88a;"></span> ${user.status}
                    </span>
                </td>
            </tr>
        `).join('');
        ...
        tbody.querySelectorAll('.admin-role-select').forEach(sel => {
            sel.addEventListener('change', (e) => {
                const targetEmail = e.target.dataset.userEmail;
                const newRole = e.target.value;
                const targetUser = adminWorkforceData.find(u => u.email === targetEmail);
                if (targetUser) {
                    targetUser.role = newRole;
                    // If current logged in user, immediately update active identity
                    if (currentUserState.email.toLowerCase() === targetEmail.toLowerCase()) {
                        updatePillDisplay({ role: newRole });
                    }
                    showToastNotification(`Role updated for ${targetUser.name} to ${newRole.toUpperCase()}!`, 'success');
                    appendAdminAudit('AUTH', `Role privilege altered: ${targetEmail} set to ${newRole.toUpperCase()}`);
                }
            });
        });

        // Freeze button
        const freezeBtn = document.getElementById('adminToggleFreezeBtn');
        const freezeStatus = document.getElementById('adminFreezeStatus');
        if (freezeBtn && freezeStatus && !freezeBtn.dataset.bound) {
            freezeBtn.dataset.bound = 'true';
            freezeBtn.addEventListener('click', () => {
                isPayrollFrozen = !isPayrollFrozen;
                if (isPayrollFrozen) {
                    freezeStatus.textContent = 'ACTIVE (FROZEN)';
                    freezeStatus.style.color = '#ff6b6b';
                    freezeBtn.style.background = 'rgba(255,107,107,0.2)';
                    freezeBtn.style.borderColor = '#ff6b6b';
                    showToastNotification('EMERGENCY PAYROLL FREEZE ACTIVATED! Ledger disbursements locked.', 'error');
                    appendAdminAudit('ALERT', 'Emergency payroll ledger freeze initiated by Administrator');
                } else {
                    freezeStatus.textContent = 'OFF';
                    freezeStatus.style.color = '';
                    freezeBtn.style.background = 'rgba(255,107,107,0.06)';
                    freezeBtn.style.borderColor = 'rgba(255,107,107,0.3)';
                    showToastNotification('Emergency payroll freeze deactivated. Normal ledger operations resumed.', 'success');
                    appendAdminAudit('SEC', 'Payroll ledger freeze revoked. Operations normal.');
                }
            });
        }

        // Flush tokens button
        const flushBtn = document.getElementById('adminFlushSessionsBtn');
        if (flushBtn && !flushBtn.dataset.bound) {
            flushBtn.dataset.bound = 'true';
            flushBtn.addEventListener('click', () => {
                showToastNotification('All active sessions and cryptographic tokens purged across cluster.', 'success');
                appendAdminAudit('KEY', 'Emergency JWT & OAuth token invalidation cycle completed.');
            });
        }

        // Strict 2FA toggle button
        const toggle2faBtn = document.getElementById('adminToggle2faBtn');
        const status2fa = document.getElementById('admin2faStatus');
        if (toggle2faBtn && status2fa && !toggle2faBtn.dataset.bound) {
            toggle2faBtn.dataset.bound = 'true';
            toggle2faBtn.addEventListener('click', () => {
                isStrict2fa = !isStrict2fa;
                status2fa.textContent = isStrict2fa ? 'ON' : 'OFF';
                status2fa.style.color = isStrict2fa ? '#62c88a' : '#f59e0b';
                showToastNotification(`Strict Enterprise 2FA Policy ${isStrict2fa ? 'ENFORCED' : 'RELAXED'}`, isStrict2fa ? 'success' : 'info');
                appendAdminAudit('SEC', `Global 2FA requirement set to: ${isStrict2fa ? 'ENFORCED' : 'OPTIONAL'}`);
            });
        }
    }

    function appendAdminAudit(tag, text) {
        const stream = document.getElementById('adminAuditStream');
        if (!stream) return;
        const line = document.createElement('div');
        line.className = 'admin-audit-line';
        const tagClass = tag === 'ALERT' ? 'sec' : (tag === 'AUTH' ? 'auth' : 'gov');
        line.innerHTML = `
            <span class="admin-audit-tag ${tagClass}">${escapeHtml(tag)}</span>
            <span>${escapeHtml(text)} [${new Date().toLocaleTimeString()}]</span>
        `;
        stream.insertBefore(line, stream.firstChild);
    }

    function openAuthProfileModal(initialTab = 'profile') {
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                <div class="modal-card helios-auth-modal-card">
                    ...
                    <div class="modal-body helios-auth-modal-body">
                        <!-- Navigation Tabs: Profile vs Login vs Admin -->
                        <div class="helios-auth-tabs">
                            ...
                            <button type="button" class="helios-auth-tab admin-tab" id="tabBtnAdmin">
                        ...
                        <div id="authLoginPanel" class="helios-auth-panel" style="display:none;">
                            ...
                            <div class="helios-quick-accounts-section">
                                ...
                                <div class="helios-quick-accounts">
                                    <button type="button" class="helios-quick-acc-btn admin" data-email="rachel_nexus@gmail.com" data-name="Nadia Rachel" data-role="admin">
                                        <div class="quick-acc-icon-wrap"><i class="fas fa-crown" style="color:#ffd700;"></i></div>
                                        <div class="quick-acc-details">
                                            <div class="quick-acc-role" style="color:#ffd700;">Executive Admin</div>
                                            <div class="quick-acc-name">Nadia Rachel</div>
                                        </div>
                                    </button>
                            ...
                            <div class="helios-auth-divider">
                                <span>Corporate Credentials Login</span>
                            </div>
                        ...
                        <!-- Panel 3: Comprehensive Admin Control Panel -->
                        <div id="authAdminPanel" class="helios-auth-panel" style="display:none;">
                            <!-- Live Workforce & System Roles Directory -->
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                                <h4 style="margin:0;font-size:13px;font-weight:700;color:#fff;display:flex;align-items:center;gap:6px;">
                                    <i class="fas fa-users-gear" style="color:#ffd700;"></i> Workforce & System Role Privileges
                                </h4>
                            </div>
                            ...
                            <div style="margin-top:14px;margin-bottom:12px;">
                                <div style="font-size:12px;font-weight:700;color:#fff;margin-bottom:8px;display:flex;align-items:center;gap:6px;">
                                    <i class="fas fa-sliders" style="color:#b68cff;"></i> Security Governance & System Controls
                                </div>
                                <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(180px, 1fr));gap:8px;">
                                    <button type="button" id="adminToggleFreezeBtn" class="btn btn-neutral" style="font-size:11.5px;padding:8px 12px;justify-content:center;border-color:rgba(255,107,107,0.3);color:#ff6b6b;background:rgba(255,107,107,0.06);">
                                        <i class="fas fa-lock" style="margin-right:6px;"></i> Payroll Freeze: <strong id="adminFreezeStatus" style="margin-left:4px;">OFF</strong>
                                    </button>
                                    <button type="button" id="adminFlushSessionsBtn" class="btn btn-neutral" style="font-size:11.5px;padding:8px 12px;justify-content:center;color:#f1f0f5;">
                                        <i class="fas fa-key" style="margin-right:6px;"></i> Force Revoke All Tokens
                                    </button>
                                    <button type="button" id="adminToggle2faBtn" class="btn btn-neutral" style="font-size:11.5px;padding:8px 12px;justify-content:center;color:#62c88a;border-color:rgba(98,200,138,0.3);background:rgba(98,200,138,0.06);">
                                        <i class="fas fa-fingerprint" style="margin-right:6px;"></i> Strict 2FA: <strong id="admin2faStatus" style="margin-left:4px;">ON</strong>
                                    </button>
                                </div>
                            </div>

                            <!-- Live Security & Audit Stream -->
                            <div style="font-size:11px;font-weight:700;color:#8b8d9e;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.04em;">
                                Live Security Audit Stream
                            </div>
                            <div class="admin-audit-box" id="adminAuditStream">
                                <div class="admin-audit-line">
                                    <span class="admin-audit-tag sec">SEC</span>
                                    <span>Zero-Trust handshake established (TLS 1.3 / AES-256-GCM)</span>
                                </div>
                                <div class="admin-audit-line">
                                    <span class="admin-audit-tag auth">AUTH</span>
                                    <span>Administrator session verified for rachel_nexus@gmail.com</span>
                                </div>
                                <div class="admin-audit-line">
                                    <span class="admin-audit-tag gov">GOV</span>
                                    <span>Multi-currency real-time exchange rates synchronized</span>
                                </div>
                                <div class="admin-audit-line">
                                    <span class="admin-audit-tag sec">AI</span>
                                    <span>Nexus AI Copilot engine connected via Gemini 3.1 Flash Lite</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            // 3-Way Tab switching: Profile, Login, Admin
            ...
            function switchAuthTab(targetTab) {
                [tabBtnProfile, tabBtnLogin, tabBtnAdmin].forEach(b => b?.classList.remove('active'));
                [profilePanel, loginPanel, adminPanel].forEach(p => { if (p) p.style.display = 'none'; });
                ...
                if (targetTab === 'admin') {
                    ...
                    const notice = document.getElementById('loginFormNotice');
            ...
            modal.switchAuthTab = switchAuthTab;
            tabBtnProfile?.addEventListener('click', () => switchAuthTab('profile'));
            tabBtnLogin?.addEventListener('click', () => switchAuthTab('login'));
            tabBtnAdmin?.addEventListener('click', () => switchAuthTab('admin'));
        ...
        if (typeof modal.switchAuthTab === 'function') {
            modal.switchAuthTab(initialTab);
        }
    ...
    function openOAuthConsentScreen(provider, meta) {
        ...
        modal.innerHTML = `
            <div class="oauth-consent-box ${provider}">
                ...
                <div class="oauth-consent-body" style="padding:20px 24px;display:flex;flex-direction:column;gap:16px;">
    ...
    function initHelios() {
        ...
        if (mobileNotifBtn) mobileNotifBtn.addEventListener('click', openNotifModal);
        syncNotificationBadges();
        ...
        if (userPill) {
            userPill.addEventListener('click', () => openAuthProfileModal('profile'));
            userPill.addEventListener('keydown', (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    openAuthProfileModal('profile');
                }
            });
        }
        fetchCurrentProfile();

        // Topbar & Navigation AI Assistant Triggers
        const topAiBtn = document.getElementById('topbarAiAssistantBtn');
        if (topAiBtn) topAiBtn.addEventListener('click', () => openAiInsightsModal());
        const mobAiBtn = document.getElementById('mobileAiBtn');
        if (mobAiBtn) mobAiBtn.addEventListener('click', () => openAiInsightsModal());
        const drawerAiBtn = document.getElementById('pnMobileAiBtn');
        if (drawerAiBtn) drawerAiBtn.addEventListener('click', () => {
            const drawer = document.getElementById('heliosMobileDrawer');
            if (drawer) drawer.classList.remove('open');
            openAiInsightsModal();
        });

        // Sidebar & Navigation Admin Panel Triggers
        const sidebarAdminBtn = document.getElementById('sidebarAdminBtn');
        if (sidebarAdminBtn) sidebarAdminBtn.addEventListener('click', () => openAuthProfileModal('admin'));
        const drawerAdminBtn = document.getElementById('pnMobileAdminBtn');
        if (drawerAdminBtn) drawerAdminBtn.addEventListener('click', () => {
            const drawer = document.getElementById('heliosMobileDrawer');
            if (drawer) drawer.classList.remove('open');
            openAuthProfileModal('admin');
        });

        // Global function exports for seamless invocation
        window.openAuthProfileModal = openAuthProfileModal;
        window.openAdminPanelModal = () => openAuthProfileModal('admin');
        window.openLoginModal = () => openAuthProfileModal('login');
